...
// =========================================================================================================================================
            ...
            case FRESH_STATE_DETAIL:
                if (hasKeyword(input, "เรียนอะไร") || hasKeyword(input, "เกี่ยวกับอะไร") || hasKeyword(input, "เรียนเกี่ยวกับ")) {        
                    ...
                    currentState = FRESH_STATE_SELECT_PROGRAM;
                    selectedProgram = ""; selectedMajor = ""; 
                    reply("กลับไปเลือกภาคใหม่ครับ: ปกติ / พิเศษ / นานาชาติ");
                }
                else if (hasKeyword(input, "อาคารเรียน") || hasKeyword(input, "ตึก") || hasKeyword(input, "อาคาร")) {
                    currentState = FRESH_STATE_Building;
                    reply("ต้องการทราบข้อมูลอาคารเรียนใช่ไหมครับ? พิมพ์ 'ใช่' เพื่อดูรายละเอียดเลยครับ");
                }
                else if (hasKeyword(input, "อุปกรณ์") || hasKeyword(input, "เครื่องมือ")) {
                    currentState = FRESH_STATE_ACCESSORIES;
                    reply("ต้องการสอบถามเรื่องอุปกรณ์การเรียนใช่ไหมครับ? (เช่น ไม้บรรทัด, เครื่องคิดเลข)");
                }
                else if (hasKeyword(input, "หอพัก") || hasKeyword(input, "หอ")) {
                    currentState = FRESH_STATE_DORMITORY;
                    reply("ต้องการสอบถามเรื่องใดเกี่ยวกับหอพักครับ?\n(เช่น จำนวนหอพัก, ราคา, ค่าน้ำค่าไฟ, วิธีจองหอ, ช่องทางติดต่อ)");
                }
                else if (hasKeyword(input, "ชุด") || hasKeyword(input, "เครื่องแต่งกาย") || hasKeyword(input, "เสื้อผ้า")){
                    currentState = FRESH_STATE_CLOTHES;
                    reply("ต้องการสอบถามเรื่องใดเกี่ยวกับเครื่องแต่งกายครับ?\n(เช่น ระเบียบการแต่งกาย, ซื้ออุปกรณ์ที่ไหน)");
                }
                else {
                ...
                // Mapshttps://github.com/PackCodeDev/ENGR-CMU-Chat-bot/edit/main/mewtwo.cpp
                case FRESH_STATE_Building:
                    if (hasKeyword(input, "อาคารเรียน") || hasKeyword(input, "อาคาร") || hasKeyword(input, "ตึก") || hasKeyword(input, "ใช่")) {
                ...
                case FRESH_STATE_ACCESSORIES:
                    ...
                    if (hasKeyword(input, "ไม้ที") || hasKeyword(input, "ไม้บรรทัด")) {                    
                ...
                case FRESH_STATE_DORMITORY:
                    ...
                    if(hasKeyword(input, "จำนวนหอพัก") || hasKeyword(input, "มีกี่หอ")){
                        ...
                        "หอพักนักศึกษาชาย (รวม 8 อาคาร) โดยจะมี\n"
                        "หอพักในกำกับ (หอธรรมดา): มี 7 อาคาร คือ หอ 1 ชาย ถึง หอ 7 ชาย\n"
                        "หอพักในกำกับเชิงพาณิชย์: มี 1 อาคาร คือ หอพักนักศึกษาชายอาคาร 40 ปี (หรือที่เรียกกันว่า หอสีชมพูฝั่งชาย)\n"
                        "หอพักนักศึกษาหญิง (รวม 11 อาคาร) โดยจะมี\n"
                        "หอพักในกำกับ (หอธรรมดา/ปรับปรุง): มี 10 อาคาร คือ หอ 1 หญิง ถึง หอ 10 หญิง \n****บางหอในกลุ่มนี้ เช่น หอ 4, 6, 7 และ 8 หญิง ได้รับการปรับปรุงเป็นหอพักแบบพรีเมียม (ห้องน้ำในตัว/กึ่งส่วนตัว) ซึ่งค่าเทอมจะสูงกว่าหอธรรมดา\n"
                        "หอพักในกำกับเชิงพาณิชย์: มี 1 อาคาร คือ หอพักนักศึกษาหญิงอาคาร 40 ปี (หอสีชมพูฝั่งหญิง ซึ่งเป็นหอยอดฮิต)\n");
                        currentState = FRESH_STATE_DORMITORY;
                        ...
                        "เทอมแรก (แรกเข้า): ประมาณ 2,400 บาท\n"
                        "เทอมถัดไป: ประมาณ 2,200 บาท\n"
                        "ลักษณะ: พัดลม, ห้องน้ำรวม (หารกันจ่ายถูกมาก เฉลี่ยเดือนละไม่กี่ร้อยบาท)\n"
                        "หอพักปรับปรุง (พรีเมียม)เช่น หอหญิง 4, 6, 7 และบางอาคารที่รีโนเวทใหม่\n"
                        "เทอมแรก (แรกเข้า): ประมาณ 10,850 บาท\n"
                        "เทอมถัดไป: ประมาณ 8,750 บาท\n"
                        "ลักษณะ: ห้องน้ำในตัวหรือกึ่งส่วนตัว, เฟอร์นิเจอร์ครบชุด, สภาพห้องใหม่กว่า\n"
                        "หอพักอาคาร 40 ปี (หอสีชมพู) หอยอดฮิตที่อยู่ใกล้ตลาดฝายหิน\n"
                        "แรกเข้า (รวมค่าประกัน): ประมาณ 10,100 - 10,850 บาท\n"
                        "เทอมถัดไป: ประมาณ 9,000 บาท\n"
                        "ลักษณะ: มีทั้งแบบพัก 2 คน หรือเหมาห้อง, มีระเบียง, ใกล้แหล่งของกิน\n");
                        currentState = FRESH_STATE_DORMITORY;
                        
                    }else if(hasKeyword(input, "ค่าน้ำ") || hasKeyword(input, "ค่าไฟ")){
                        reply("หอธรรมดา: มักรวมอยู่ในค่าเทอมแล้ว (ยกเว้นเครื่องใช้ไฟฟ้าพิเศษ)\n"
                        "หอปรับปรุง/หอ 40 ปี: จ่ายตามมิเตอร์จริง (ค่าไฟหน่วยละประมาณ 6 บาท, ค่าน้ำเหมาจ่ายหรือตามหน่วย) ต้องจ่ายทุกเดือนภายในวันที่ 5\n"
                        "ค่าธรรมเนียมเครื่องใช้ไฟฟ้า: หากนำเครื่องใช้ไฟฟ้ามาเอง (เช่น ตู้เย็น, หม้อหุงข้าว) ต้องลงทะเบียนจ่ายเพิ่มประมาณ 100 - 300 บาท ต่อเทอม\n"
                        "ค่าประกันของเสียหาย: จ่ายครั้งแรกที่เข้าพัก (ประมาณ 500 - 2,000 บาท ขึ้นอยู่กับประเภทหอ) และจะได้คืนเมื่อย้ายออกหากห้องไม่เสียหาย\n");
                        currentState = FRESH_STATE_DORMITORY;
                        ...
                        "1.สร้างบัญชีผู้ใช้งาน (ครั้งแรก): ใช้นักศึกษาใหม่ (ปี 1) ต้องเข้าไปลงทะเบียนสร้างบัญชีโดยใช้ เลขประจำตัวประชาชน และ วันเดือนปีเกิด \n****สำคัญ: ต้องระบุเพศให้ถูกต้อง เพราะระบบจะล็อกหอพักตามเพศไว้ครับ\n"
                        "2.ล็อกอินเข้าระบบจอง: เมื่อถึงวันและเวลาที่ประกาศเปิดจอง (มักจะเป็นช่วงหลังประกาศผล TCAS ในแต่ละรอบ) ให้รีบล็อกอินเข้าสู่ระบบ\n"
                        "3.ระบบจะแสดงรายชื่อหอพักที่ยังว่างอยู่ คุณสามารถเลือกเลขห้องและเลขเตียงที่ต้องการได้ทันที \n****ตรงนี้หอปรับปรุงและหอ 40 ปี จะเต็มเร็วมากภายในไม่กี่นาที แนะนำให้เล็งห้องในใจไว้ก่อนครับ \n"
                        "4.ยืนยันการจองและพิมพ์ใบชำระเงิน: เมื่อจองสำเร็จ ระบบจะให้พิมพ์ใบแจ้งหนี้เพื่อไปชำระเงินที่ธนาคารหรือผ่านแอปฯ ธนาคารตามที่กำหนด\n");
                        currentState = FRESH_STATE_DORMITORY;
                ...
                case FRESH_STATE_CLOTHES:
                    ...
                    if(hasKeyword(input, "ชุดที่ใส่") || hasKeyword(input, "การแต่งกาย")){
                        reply("การแต่งกายจะมีดังนี้ครับ\n"
                        "ชุดนักศึกษาถูกระเบียบ (ชุดพิธีการ): ใช้สำหรับงานพิธีการสำคัญ เช่น วันปฐมนิเทศ, วันไหว้ครู, และวันสอบ\n"
